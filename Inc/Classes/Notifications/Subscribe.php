...
if ( ! class_exists( 'Subscribe' ) ) {
	...
	class Subscribe extends Notice {
		...
		public function __construct() {
			parent::__construct();
			add_action( 'wp_ajax_jltctwr_subscribe', array( $this, 'jltctwr_subscribe' ) );
		}
		 ...
		 * @author Jewel Theme <support@jeweltheme.com>
		 */
		public function jltctwr_subscribe() {
			check_ajax_referer( 'jltctwr_subscribe_nonce' );
		...
		public function notice_content() {
			...
			?>
			<div class="notice notice-jltctwr is-dismissible notice-plugin-review notice-<?php echo esc_attr( $this->color ); ?> jltctwr-notice-<?php echo esc_attr( $this->get_id() ); ?>">
				<button type="button" class="notice-dismiss jltctwr-notice-dismiss"></button>
				<img width="70" height="70" src="<?php echo esc_url( JLTCTWR_IMAGES . '/logo.png' ); ?>" alt="<?php esc_attr_e( 'Logo', 'wp-awesome-city-weather-report' ); ?>">
				<div class="jltctwr-subscribe-content">
					<?php $this->set_title(); ?>
					<form style="display:flex">
						<div class="jltctwr-plugin-subscribe-input">
						...
						<button type="submit" class="button button-primary jltctwr-subscribe-btn"><?php esc_html_e( 'Get Discount', 'wp-awesome-city-weather-report' ); ?></button>
					</form>
				</div>
			</div>
			<?php
		...
		public function core_script( $trigger_time ) {
			...
			<script>
				jQuery(document).on('submit', '.jltctwr-notice-subscribe .jltctwr-subscribe-content form', function(e) {
					...
					let formWrapper = form.closest('.jltctwr-subscribe-content');
					...
					jQuery.ajax({
							...
							crossDomain: true,
							data: {
								action: 'jltctwr_subscribe',
								_wpnonce: '<?php echo esc_js( wp_create_nonce( 'jltctwr_subscribe_nonce' ) ); ?>',
								name: name,
								email: email,
							}
						})
						.done(function(response) {
							formWrapper.hide().after('<p class="jltctwr--notice-message"><strong>' + response.data + '</strong><p>');
							let subsTimeout = setTimeout(function() {
								jltctwr_notice_action(null, form, 'disable');
